...
red_alert "ПЕРЕУСТАНАВЛИВАЕТ БОТА ЗАНОВО, В ТОМ ЧИСЛЕ УДАЛЯЯ ВСЕ ПРЕДЫДУЩИЕ ДАННЫЕ!."
...
echo "Пожалуйста, введите публичный ключ предыдущего сервера для установления связи в процессе миграции:"
read -r public_key
